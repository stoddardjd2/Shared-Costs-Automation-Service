...
import { Info as InfoIcon } from "lucide-react"; // the info-in-a-circle icon
...
const getPaymentMethods = (initial) => [
  {
    id: "venmo",
    name: "Venmo",
    bgColor: "#3D95CE",
...
const StatusBadge = ({ status = "confirmed" }) => {
  ...
  return (
    ...
    >
      ...
      Payment Status: {isConfirmed ? "Confirmed" : "Paid"}
    </div>
  );
};

const PaymentSummary = ({ initial, paidDate = null }) => {
  const summaryItems = [
    { label: "Amount Paid", value: formatCurrency(initial.paymentAmount) },
    { label: "Payment For", value: initial.paymentName || "Payment" },
    { label: "Paid By", value: initial.userName || "You" },
    {
      label: paidDate ? "Date Paid" : "Date Confirmed",
      value: paidDate || getCurrentDate(),
    },
  ];
...
const SuccessMessage = ({ initial, isAlreadyPaid = false }) => (
...
export default function PaymentPage() {
  ...
  console.log(
    " params.get(allowMarkAsPaidForEveryone)",
    params.get("allowMarkAsPaidForEveryone")
  );
  const formattedDate = formatDate(initial.dueDate);
  const paymentMethods = useMemo(() => getPaymentMethods(initial), [initial]);
    ...
    return (
      <PageLayout>
        ...
        <div className="p-8">
          ...
          <PaymentSummary initial={initial} paidDate={paidDate} />
          <SuccessMessage initial={initial} isAlreadyPaid={isAlreadyPaid} />
          <CloseButton />
        </div>
      </PageLayout>
    );
  ...
  const paymentDetails = [
    { label: "For", value: initial.userName || "(you)" },
    { label: "Requested by", value: initial.requesterName || "—" },
    { label: "Charge Name", value: initial.paymentName || "—" },
    {
      label: "Amount Due",
      value: initial.paymentAmount
        ? formatCurrency(initial.paymentAmount)
        : "—",
      highlight: true,
    },
    { label: "Due Date", value: formattedDate || "" },
    { label: "Frequency", value: initial.frequency || "One-time" },
  ];
  ...
  return (
    <PageLayout>
      ...
      >
        ...
        {/* Instructions Notice */}
        {initial.allowMarkAsPaidForEveryone && (
          <>
