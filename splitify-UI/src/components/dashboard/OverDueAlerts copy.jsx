...
const OverdueAlerts = () => {
  ...
  const isOverdue = (participant, dueDate) => {
    ...
    const isNotPaid = participant?.amount > participant.paymentAmount || participant.markedAsPaid;
                ...
                >
                  ...
                  <div className="space-y-2">
                    {overdueParticipants.map((participant, index) => {
                      ...
                      if (
                        !request.participants[index]?.markedAsPaid ||
                        request.participants[index].paymentAmount <
                          request.participants[index].amount
                      )
                        return;
                      return (
                        <PaymentHistoryParticipantDetails
                          key={participantId._id + index}
