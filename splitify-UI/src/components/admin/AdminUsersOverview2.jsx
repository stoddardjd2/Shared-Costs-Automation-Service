...
export default function AdminUsersOverview() {
  ...
  function getActivityStatus(lastActive) {
    ...
    return { label: "inactive", color: "bg-red-100 text-red-700 border-red-300" };
  }
      ...
      <header className="sticky top-0 z-40 backdrop-blur supports-[backdrop-filter]:bg-white/70 bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Search, filter, and explore user metrics at a glance.</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <section>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Kpi title="Total Users" value={counts.total} sub={`${counts.new30d} joined last 30d`} />
            <Kpi title="New (7d)" value={counts.new7d} sub="Signups last 7 days" />
            <Kpi title="Active (24h)" value={counts.active24h} sub={`${counts.active7d} active 7d`} />
            <Kpi title="Total Requests" value={counts.totalRequests} sub="Across all users" />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card title="Daily Signups (14d)">
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailySignups} margin={{ left: 8, right: 8, top: 10 }}>
          ...
          <Card title="Plan Breakdown">
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={planPie} dataKey="value" nameKey="name" outerRadius={90}>
                    {planPie.map((entry, idx) => (
                      <Cell
                        key={`cell-${idx}`}
                        fill={PLAN_COLORS[entry.name?.toLowerCase()] || PLAN_COLORS.unknown}
                      />
                    ))}
                  </Pie>
                  <Legend />
            ...
            <div className="mt-3 text-sm text-gray-500">
              <span className="mr-4">Stripe Linked: <b>{counts.withStripe}</b></span>
              <span>Plaid Enabled: <b>{counts.withPlaid}</b></span>
            </div>
          </Card>
          ...
          <Card title="Active Users">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-sm rounded-lg border ${
                    activeWindow === b.k ? "bg-blue-600 text-white border-blue-600" : "bg-white border-gray-300"
                  }`}
                >
                  {b.label}
                </button>
            ...
            <ul className="divide-y divide-gray-200 rounded-xl border border-gray-200 overflow-hidden bg-white">
              {activeUsersList.map((u, i) => (
                <li key={i} className="p-3 flex items-center justify-between gap-4">
                  <UserCell user={u} subtitle={`Last active ${fmtRelative(asDate(u.lastActive), now)}`} />
                  <span className="text-xs text-gray-500">{(u.plan || "—").toUpperCase()}</span>
                </li>
              ))}
              {activeUsersList.length === 0 && <EmptyRow text="No active users in this window." />}
            </ul>
          </Card>
        </section>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card title="New Users (7d)">
            <ul className="divide-y divide-gray-200 rounded-xl border border-gray-200 overflow-hidden bg-white">
              {newUsersList.map((u, i) => (
                <li key={i} className="p-3 flex items-center justify-between gap-4">
                  <UserCell user={u} subtitle={`Joined ${fmtRelative(asDate(u.createdAt), now)}`} />
                  <span className="text-xs text-gray-500">{(u.plan || "—").toUpperCase()}</span>
                </li>
              ))}
              {newUsersList.length === 0 && <EmptyRow text="No signups in the last 7 days." />}
            </ul>
          </Card>
          ...
          <Card title="Latest Requests">
            <ul className="divide-y divide-gray-200 rounded-xl border border-gray-200 overflow-hidden bg-white">
                ...
                <li key={i} className="p-3 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">
                      Owner: {r._ownerName} · Amount: ${Number(r.amount || 0).toFixed(2)} · Created {fmtRelative(asDate(r.createdAt), now)}
                    </div>
                  </div>
              ...
              {latestRequests.length === 0 && <EmptyRow text="No recent requests." />}
            </ul>
          </Card>
        </section>
                ...
                <tbody className="divide-y divide-gray-100">
                  {usersWithPlaid.map((u, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      <Td><UserTiny user={u} /></Td>
                      <Td>{u.email}</Td>
                      <Td>{fmtDate(asDate(u?.plaid?.enabledOn))}</Td>
                      <Td>{fmtDate(asDate(u?.plaid?.lastUsed))}</Td>
                      <Td><code className="text-gray-500">{mask(u?.plaid?.accessToken)}</code></Td>
                    </tr>
                  ))}
                  {usersWithPlaid.length === 0 && (
                    <tr><td colSpan={5} className="p-6 text-center text-gray-500">No users with Plaid enabled.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {usersStripe.map((u, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <Td>{Array.isArray(u.requests) ? u.requests.filter((r)=>r && r._id && !r.$oid).length : 0}</Td>
                    </tr>
                  ))}
                  {usersStripe.length === 0 && (
                    <tr><td colSpan={5} className="p-6 text-center text-gray-500">No users with Stripe.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {allRequests.map((r, i) => (
                    <tr key={i} className="hover:bg-gray-50">
                      ...
                      <Td className="max-w-[220px] truncate">{r._ownerName} <span className="text-xs text-gray-400">({r._ownerEmail})</span></Td>
                      <Td>${Number(r.amount || 0).toFixed(2)}</Td>
                      <Td className="uppercase">{r.isRecurring ? (r.frequency || "recurring") : "one-time"}</Td>
                      <Td>{Array.isArray(r.participants) ? r.participants.length : 0}</Td>
                  ...
                  {allRequests.length === 0 && (
                    <tr><td colSpan={9} className="p-6 text-center text-gray-500">No requests found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-100">
                  ...
                  {requestsByUser.length === 0 && (
                    <tr><td colSpan={4} className="p-6 text-center text-gray-500">No users with requests.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
                    ...
                    <tr key={i} className="hover:bg-gray-50">
                      <Td>
                        <div className="flex items-center gap-3">
                          <Avatar src={u?.OAuth?.google?.picture} fallback={initials(u?.name)} />
                          <div>
                            <div className="font-medium">{u.name || "—"}</div>
                            <div className="text-xs text-gray-500">Google: {mask(u?.OAuth?.google?.sub, 6)}</div>
                          </div>
                        </div>
                      </Td>
                      <Td>{u.email}</Td>
                      ...
                      <Td>
                        {(() => {
                          const status = getActivityStatus(asDate(u.lastActive));
                          return (
                            <span className={`inline-flex items-center px-2 py-1 rounded-lg text-[11px] font-medium border ${status.color}`}>
                              {status.label}
                            </span>
                          );
                        })()}
                      ...
                      <Td>{Array.isArray(u.contacts) ? u.contacts.length : 0}</Td>
                      <Td>{Array.isArray(u.requests) ? u.requests.filter((r)=>r && r._id && !r.$oid).length : 0}</Td>
                    </tr>
                  ))}
                  {filtered.length === 0 && (
                    <tr>
                      <td colSpan={9} className="p-6 text-center text-gray-500">No users match the current filters.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <footer className="py-10 text-center text-xs text-gray-400">Splitify Admin · v1.1</footer>
      </main>
    </div>
  );
}
...
function Kpi({ title, value, sub }) {
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4">
      ...
      <div className="mt-1 text-3xl font-semibold">{Number(value).toLocaleString()}</div>
...
function Td({ children, className = "" }) {
  return <td className={`px-4 py-2 align-middle whitespace-nowrap ${className}`}>{children}</td>;
}
...
function UserCell({ user, subtitle }) {
  return (
    <div className="flex items-center gap-3 min-w-0">
      <Avatar src={user?.OAuth?.google?.picture} fallback={initials(user?.name)} />
      <div className="min-w-0">
